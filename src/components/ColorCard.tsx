...
const CopyButton = memo<CopyButtonProps>(({ text, type, isCopied, onCopy }) => (
  <button
    ...
    className="flex items-center justify-center gap-2 rounded bg-slate-700 px-3 py-1.5 text-sm text-slate-300 transition-colors hover:bg-slate-600"
  >
...
const FormatDisplay = memo<FormatDisplayProps>(({ text, type, isCopied, onCopy }) => (
  <div className="flex items-center gap-2">
    ...
    >
      {isCopied ? <Check size={12} className="text-green-400" /> : <Clipboard size={12} />}
    </button>
  </div>
));
  ...
  return (
    <div
      className={`relative flex flex-col items-center gap-4 rounded-xl p-4 transition-all duration-300 md:flex-row ${
        isClosest
          ? 'scale-[1.02] bg-slate-800 shadow-lg ring-2 ring-indigo-500'
    ...
    >
      ...
      <div className="w-full flex-1 text-center md:text-left">
        {/* 顏色名稱 */}
        <h3 className="mb-1 text-xl font-bold text-white">{color.class}</h3>
